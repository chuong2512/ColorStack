...
public class Holder : MonoBehaviour,IInitializable<IEnumerable<Ball>>
{
	...
	public bool IsPending
	{
		...
		set
		{
			...
			else if(PendingBall!=null)
			{
				...
				SetIdex(PendingBall, _balls.Count - 1);
				PendingBall=null;
				PlayClipIfCan(_popClip);
			}
	...
	public void Move(Ball ball)
	{
		...
		SetIdex(ball,_balls.Count-1);
	}


	public Vector2 GetBallPosition(int index) { return transform.TransformPoint((index+0.5f)*2*_ballRadius*Vector2.up); }

	public void SetIdex(Ball ball,int index) { ball.GetComponentInChildren<SpriteRenderer>().sortingOrder=56+index; }
